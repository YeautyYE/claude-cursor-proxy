...
| `CCP_CURSOR_LIVE_CONCURRENCY` | `32` | Fair cap for active Cursor generations (1–128) |
...
| grok-build `Conflict (409) - Cursor live open timed out after 20s` then many `A Cursor live run is already active` | Update to ≥0.1.60 and restart serve. H2 first-open waits 90s; a definitely Starting/Running local slot returns HTTP 503 + `Retry-After`. A response-less open whose upstream acceptance is genuinely unknown remains fail-closed as 409 to prevent a duplicate Run. Start a fresh Grok session after upgrading. |
