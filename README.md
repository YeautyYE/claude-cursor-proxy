...
| Claude Code `unexpected internal error` then `live open timed out after 10s` (often `gemini-3.6-flash-high`) | Update to ≥0.1.49 and restart serve. HTTP/1 ResumeAction now uses the first-open budget, not a flat 10s. Live-open concurrency grows 4→128 without setting `CCP_CURSOR_LIVE_OPEN_CONCURRENCY`. |
| grok-build `Conflict (409) - error sending request` / `live open timed out after 20s`, or Claude Code `Agent type 'gemini-3.6-flash-high' not found` | Update to ≥0.1.50 and restart serve. The proxy retries H2/connect misses on HTTP/1 before 409; High Load 429s are not retried; Agent/Task model slugs remap to `general-purpose`. |
...
| grok-build ends with `Cursor finished this turn without text or tool calls`, or reports that `workflow` was intercepted/renamed | Update to ≥0.1.52 and restart serve. Empty Cursor turns retry instead of becoming successful assistant text; malformed control XML is quarantined; exact workflow/skill casing is preserved. |
| grok-build/Grok 4.6 fan-out shows many failed subagents, repeats completed tools, or stalls without tokens | Update to ≥0.1.53 and restart serve. Split parallel tool outputs resume as one batch; empty-turn recovery is bounded; active generations are capped and tool-result resumptions have priority. Start a fresh Grok session after upgrading. |
