...
- Treat Cursor policy/billing 429s (`Free plans can only use Auto`, unpaid invoice, `user_rate_limit_exceeded`) as terminal instead of silently retrying them in place. When the CLI hot-swapped to a different account, the failed start now automatically retries under the new account (up to 2 swaps) instead of passing the stale account's 429 through to the client.
- Make `Succeeded` tombstone claiming atomic (release + reserve in one registry transition) so an identical retry racing a new operation can neither double-execute nor observe a half-released slot; a completed operation without a stored replay now fails closed as 409. Replay-bearing tombstones live 300 seconds and aggregate replay memory is bounded process-wide with lazy, bounded streaming to the retry.
- Serialize Cursor auth refresh behind a single-flight lock with compare-and-swap against the on-disk account: concurrent refreshes can no longer splice tokens across a hot account switch, and refresh HTTP now runs on the blocking pool instead of stalling async workers.
- Raise local concurrency ceilings for large agent fan-outs (4 windows x 256-512 subagents): default live generations 64 -> 1024 (cap 8192), live runs 256 -> 4096 (cap 16384), resume reserve 8 -> 64, interactive reserve 16 -> 128, recovery opens 4 -> 16, H2 shards 4 -> 16 (cap 64). Admission waits now share one deadline across run and generation permits, and local admission backpressure surfaces as 503 with `Retry-After` instead of a wrapped 502.
- Stop truncating MCP tool descriptions at 240 characters (now a 16 KB guard). Claude Code's Workflow/Task/Skill descriptions enumerate the valid workflow, agent, and skill names; the cap cut those lists off and function-calling models (gemini) hallucinated names like `asr-review-workflow`.
- Teach the prompt tool dump the exact Cursor MCP catalog names (`mcp_claude-local_<tool>`) when the MCP catalog is on the wire, instead of bare Anthropic names that Cursor rejects with `Tool not found`. grok-build requests keep exact grok wire names.
- Advertise Claude Code's `Task` tool on the MCP catalog (exact-case, Claude-Code-only) and map catalog `Task` calls back to the client's task tool, so function-calling models can actually spawn subagents; a provider guard keeps MCP Task from double-queuing through the native-Task branch.
- `cursor login` / `logout` now explain hot-swap semantics: which account was replaced, that in-flight runs keep their tokens, and that new requests use the new account without a `serve` restart.
