...
/// Claude Code's `/compact` request is a regular Messages call on newer
/// clients.  Keep a Gemini/Sand compaction turn on the summary-only path even
/// when the request also carries the normal Claude tool catalog, and promote a
/// reasoning-only Cursor response into visible summary text.
#[tokio::test]
async fn gemini_compaction_helper_isolated_and_reasoning_is_visible() {
    use axum::{Router, body::Bytes, routing::post};
    use claude_cursor_proxy::provider::{Provider, RequestContext};
    use claude_cursor_proxy::providers::cursor::CursorProvider;
    use claude_cursor_proxy::providers::cursor::connect::{
        ConnectFrameDecoder, encode_connect_frame,
    };
    ...
    let response_body = {
        let msg = AgentServerMessage {
            ...
            interaction_update: Some(InteractionUpdate {
                ...
                tool_call_completed: None,
                thinking_delta: Some(ThinkingDelta {
                    text: "summary generated by Gemini reasoning".into(),
                }),
                thinking_completed: None,
                text_delta: None,
                token_delta: None,
        ...
        let mut payload = Vec::new();
        msg.encode(&mut payload).unwrap();
        let mut bytes = encode_connect_frame(&payload, 0).to_vec();
        bytes.extend_from_slice(&encode_connect_frame(b"", 2));
        bytes
    };
    let app = Router::new().route(
        "/agent.v1.AgentService/Run",
        post(move |body: Bytes| {
            ...
            let response_body = response_body.clone();
            async move {
                *observed_handler.lock().unwrap_or_else(|e| e.into_inner()) = Some(body.to_vec());
                (
                    [(
                        axum::http::header::CONTENT_TYPE,
                        "application/connect+proto",
    ...
    unsafe {
        std::env::set_var("CCP_CURSOR_BASE_URL", &base_url);
    ...
    let provider = CursorProvider::new();
    let body: claude_cursor_proxy::anthropic::schema::MessagesRequest =
        ...
        .unwrap();
    let ctx = RequestContext {
        req_id: "gemini-compaction-test".into(),
        client_request_id: None,
        stainless_helper: Some("compaction".into()),
        session_id: None,
        session_seq: None,
        provider: "cursor".into(),
        traffic: None,
    ...
    assert_eq!(
        json["content"][0]["text"],
        "summary generated by Gemini reasoning"
    );
    assert_eq!(json["stop_reason"], "end_turn");

    let request_body = observed
        .lock()
        .unwrap_or_else(|e| e.into_inner())
        .clone()
        .expect("mock upstream captured Run request");
    let mut decoder = ConnectFrameDecoder::new();
    let frames = decoder.push(&request_body).unwrap();
    assert_eq!(frames.len(), 1);
    let request = AgentClientMessage::decode(frames[0].payload.as_ref()).unwrap();
    let run = request.run_request.expect("RunRequest");
    assert!(
