...
pub fn cursor_sand_policy() -> SandRoutingPolicy {
    ...
    let Some(cursor) = read_file_config(&paths::config_dir()).and_then(|file| file.cursor) else {
        // Sand is opt-in per model. A config without Cursor settings has no
        // model-specific Sand rules yet.
        return SandRoutingPolicy::empty();
    };
    let Some(models) = cursor.sand_models else {
        // Unrelated Cursor settings do not imply a Sand route for any model.
        return SandRoutingPolicy::empty();
    };
...
mod tests {
    ...
    fn sand_policy_reads_config_and_env_overrides_it() {
        ...
        // Environment rules are authoritative: an unmatched Fable request
        // remains on the configured default identity instead of being
        // promoted to Sand.
    ...
    #[test]
    fn missing_sand_models_field_defaults_to_empty_policy() {
        let _guard = ENV_LOCK.lock().unwrap();
        clear_env();
        let config = tempfile::TempDir::new().unwrap();
        std::fs::write(
    ...
    #[test]
    fn explicit_empty_sand_policy_keeps_fable_on_default_identity() {
        let _guard = ENV_LOCK.lock().unwrap();
        clear_env();
        let config = tempfile::TempDir::new().unwrap();
        std::fs::write(
